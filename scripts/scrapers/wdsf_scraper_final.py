        ...
        for month_idx in range(12):
            ...
            for item in items:
                try:
                    ...
                    # We look for the competitions container which holds multiple spans
                    ...
                    if date_elem and name_elem:
                        # NEW: get_text() on the container captures all spans inside 
                        # (Group, AgeGroup, Discipline, Gender, etc.)
                        name_text = name_elem.get_text(separator=" ", strip=True)
                        name_text = " ".join(name_text.split()) # Clean double spaces
